...
| 10 | Installability: the registry alone boots without an area and cannot be schema'd without one; the migration tool arrives with the bundle, and so do the versions that create its three tables | `Integration/InstallabilityTest` |
...
| 14 | The facts ledger keeps one row per subject, figure and period, replaced by the next run; a month and a figure that does not add up read their own row, an additive quarter or year is the sum of its months up to the first gap and as old as its oldest open month; no figure reads as null | `Integration/Facts/FigureFactLedgerTest` |
| 15 | `uhifadhi:facts:rebuild` asks every provider (or one module's, or one subject's) for every month of a range and the quarters and years it touches for figures that do not add up, files only what it asked, and a second run replaces the first | `Integration/Facts/FactsRebuildCommandTest` |
| 16 | The schedule recomputes the periods open now, closes a period that has just ended once, and never recomputes a closed period again; its tick queues the work, on the `default` schedule at the configured cadence, beside the installation's own tasks; a message carrying the queue marker goes to `async` | `Integration/Facts/RecomputeOpenFactsTest` + `Integration/Facts/FactsScheduleTest` + `Integration/Facts/QueuedFactsTest` |
