...
namespace Uhifadhi\Bundle\TeamBundle\MessageHandler;
 ...
 *
 * IT RUNS IN THE WORKER. Six months times every department times every
 * figure the module publishes is work that grows with the installation, so
 * the install queues it ({@see \Uhifadhi\Bundle\TeamBundle\EventListener\ModuleHistoryListener})
 * and returns; this is what the worker does with it.
 *
 * @see https://symfony.com/doc/current/messenger.html#manually-configuring-handlers — without autoconfiguration a handler is tagged 'messenger.message_handler' with the class it handles
 * @see vendor/symfony/messenger/DependencyInjection/MessengerPass.php — the `handles` attribute config/services.php tags this with
 */
final readonly class BackfillModuleHistoryHandler
{
    /**
     * HOW FAR BACK A NEW MODULE IS ASKED.
     *
     * The six the sparkline draws, and no further: the periods beyond the
     * run are not drawn anywhere.
     */
    ...
    public function __invoke(BackfillModuleHistory $message): void
    {
        ...
        foreach (PerformanceHistory::monthsEndingAt($closed, self::PERIODS) as $month) {
            ...
            foreach ($departments as $department) {
                foreach ($this->performance->kpisFor($department, $asked) as $kpi) {
                    if ($kpi->moduleSlug !== $message->slug) {
                        continue;
                    }
