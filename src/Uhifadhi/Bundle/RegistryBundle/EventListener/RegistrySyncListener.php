...
final class RegistrySyncListener implements ServiceSubscriberInterface
{
     ...
     *
     * `container.build_id` IS NOT A DOCUMENTED PARAMETER — no page of
     * symfony.com names it, and the file that carries it says so of itself
     * ("auto-generated by the Symfony Dependency Injection Component for
     * internal use"). The source is the whole of the warrant, so the source is
     * quoted: the dumper writes it as a BUILD parameter beside the build hash
     * and the build time,
     *
     *   "'container.build_id' => '$id'"  …  "$id = hash('crc32', $hash.$time);"
     *
     *   @see vendor/symfony/dependency-injection/Dumper/PhpDumper.php — dump()
     *
     * so it changes whenever the container is rebuilt (the hash covers the
     * dumped content, the time covers a rebuild of identical content). It
     * reaches `hasParameter()`/`getParameter()` through `$this->buildParameters`
     * — a branch the dumper STRIPS from a single-file dump:
     *
     *   "if (!$this->asFiles) { $code = preg_replace('/^.*buildParameters.*…/m', '', $code); }"
     *   @see vendor/symfony/dependency-injection/Dumper/PhpDumper.php — addMethods()
     *
     * which is why `has()` is asked before `get()` below rather than assumed.
     * A kernel always dumps as files —
     *
     *   "'as_files' => true,"
     *   @see vendor/symfony/dependency-injection/Kernel/KernelTrait.php — dumpContainer()
     *
     * — so in an installation the stamp is named after the build; in any other
     * host that is not true, the name falls back to 'build' and the listener
     * keeps working, one stamp for the cache directory instead of one per build.
     *
     * The bag is fetched, not injected: `ContainerBagInterface` is an alias of
     * the framework's own `parameter_bag` service, so it is a legal subscribed
     * service id.
     *   "->alias(ContainerBagInterface::class, 'parameter_bag')"
     *   @see vendor/symfony/dependency-injection/Kernel/Resources/config/services.php
     */
