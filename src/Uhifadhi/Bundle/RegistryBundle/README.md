...
| **The queue's table.** `messenger_messages` | `migrations/Version20260925210000.php` — the DSN's `auto_setup=0` leaves the schema to the migrations; `EventListener/QueueTableSchemaListener.php` declares it to the schema tool with the transport's own definition, so a kernel with no Doctrine transport is not offered a `DROP TABLE` |
