...
| 13 | The catalogue is reconciled at the end of a console command, once per build — a deploy migrates and warms up, and the first of the two does it; a web request reconciles nothing and opens no connection, and a command run before the first migration is not remembered as done | `Integration/Sync/RegistrySyncListenerTest` + `Integration/Sync/PristineCacheWarmUpTest` |
